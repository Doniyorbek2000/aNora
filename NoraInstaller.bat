...
if exist "dist\Nora_logo.png" (
    copy /Y "dist\Nora_logo.png" "%TARGET_DIR%\Nora_logo.png" >nul
)
...
if exist "logo.ico" (
    copy /Y "logo.ico" "%TARGET_DIR%\logo.ico" >nul
) else if exist "dist\logo.ico" (
    copy /Y "dist\logo.ico" "%TARGET_DIR%\logo.ico" >nul
)

echo [3/4] Ishchi stolga yorliq (Shortcut) yaratish...
powershell -NoProfile -Command "$WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut(\"$HOME\Desktop\NORA Personal AI.lnk\"); $Shortcut.TargetPath = \"$env:LOCALAPPDATA\Nora\Nora.exe\"; $Shortcut.WorkingDirectory = \"$env:LOCALAPPDATA\Nora\"; $Shortcut.IconLocation = \"$env:LOCALAPPDATA\Nora\logo.ico\"; $Shortcut.Description = \"NORA - Intelligent Personal Assistant\"; $Shortcut.Save();"

echo [4/4] Avtomatik ishga tushirish ro'yxatiga qo'shish (Startup)...
powershell -NoProfile -Command "$WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut(\"$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\NoraStartup.lnk\"); $Shortcut.TargetPath = \"$env:LOCALAPPDATA\Nora\Nora.exe\"; $Shortcut.WorkingDirectory = \"$env:LOCALAPPDATA\Nora\"; $Shortcut.IconLocation = \"$env:LOCALAPPDATA\Nora\logo.ico\"; $Shortcut.Save();"
